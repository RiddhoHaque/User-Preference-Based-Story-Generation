...
Original file is located at
    https://colab.research.google.com/drive/1AeooKHfqP-j1yLgqXssYIqjgzExu9in1
"""
...
class DecoderLSTM(nn.Module):
  ...
  def forward(self, input, hidden, encoder_out):
    ...
    att_temp = self.attention_layer(torch.cat((embeded[0], hidden[0][0]), 1))
    ...
    out = F.relu(att_temp)
    out, (h, c) = self.lstm(out, hidden)

    out = F.log_softmax(self.output_layer(out[0]), 1)
    return out, (h, c), att_w
...
def train_sgd(inp, trg, encoder, decoder, encoder_opt, decoder_opt, 
  ...
  if use_forcing:
    for i, x in enumerate(trg):
      out, (h, c), att_w = decoder(dec_inp, (h, c), encoder_out)
...
hidden_size = 256
learning_rate = 0.01
#encoder = EncoderLSTM(inp_size, hidden_size).to(device)
#decoder = DecoderLSTM(hidden_size, out_size, MAX_LENGTH, dropout=0.1).to(device)
...
def comp_train(epochs = [2, 3, 5, 7, 9, 11, 13, 15]):
  ...
  for ep in epochs:
    ...
    for it in range(ep):
      ...
      val_loss, all_bleu = validation(encoder, decoder, test_data, criterion, lr=learning_rate) # here replace test_data with val_data iterator
...
showPlot(bleu_scores, epochs, 'plots/lstmbleu.png', 'BLEU')
showPlot(val_losses, epochs, 'plots/lstmval.png', 'Test Loss')
...
torch.save(encoder.state_dict(), 'models/encoderLSTM.pt')

torch.save(decoder.state_dict(), 'models/decoderLSTM.pt')

load_encoder = EncoderLSTM(inp_size, hidden_size).to(device)
load_decoder = DecoderLSTM(hidden_size, out_size, MAX_LENGTH, dropout=0.1).to(device)

load_encoder.load_state_dict(torch.load('models/encoderLSTM.pt'))
load_decoder.load_state_dict(torch.load('models/decoderLSTM.pt'))
